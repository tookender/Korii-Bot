...
class ViewPaginator(discord.ui.View):
    ...
    async def go_to_current_page(self, interaction: discord.Interaction, _):
        pass

    @discord.ui.button(emoji="<:next:921408056766636073>", style=discord.ButtonStyle.grey)
    async def go_to_next_page(self, interaction: discord.Interaction, _):
        """go to the next page"""
        await self.show_checked_page(interaction, self.current_page + 1)

    @discord.ui.button(emoji="<:last:921408062886146088>", style=discord.ButtonStyle.grey)
    async def go_to_last_page(self, interaction: discord.Interaction, _):
        """go to the last page"""
        # The call here is safe because it's guarded by skip_if
        await self.show_page(interaction, self.source.get_max_pages() - 1)  # type: ignore

    @discord.ui.button(label="Skip to page...", style=discord.ButtonStyle.grey)
    async def numbered_page(self, interaction: discord.Interaction, _):
    ...
    async def stop_pages(self, interaction: discord.Interaction, _):
...
class SimplePages(ViewPaginator):
    ...
    def __init__(self, entries, *, ctx: commands.Context, per_page: int = 12):
        super().__init__(SimplePageSource(embed=discord.Embed(colour=discord.Colour.blurple()), entries=entries, per_page=per_page), ctx=ctx)
        self.embed = discord.Embed(colour=discord.Colour.blurple())
...
Commands usable by you (in this server): {self.usable_commands}
        ...
        embed.add_field(
            name=f"__**Available Commands ({self.total_commands})**__",
            value=f"""
{text}
```yaml
{val}
