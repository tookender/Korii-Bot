...
invalidated_webhook = (
    "https://canary.discord.com/api/webhooks/000000000000000000/_LQ1qItzrwhNj47TZEagmEgnjBJhCeLIIAE48M61S3XojN5bQuq8JM_kjv4cwCglYJlp"
)
...
class LoggingBase(commands.Cog):
    ...
    async def create_and_deliver(self, embeds: typing.List[discord.Embed], deliver_type: str, guild_id: int):
        ...
        channel_ids = await self.bot.pool.fetchrow(f"SELECT * FROM log_channels WHERE guild_id = $1", guild_id)
        if not channel_ids:
            return
        ...
        if channel.permissions_for(channel.guild.me).manage_webhooks:
            ...
            for w in webhooks_list:
                ...
                webhook = await channel.create_webhook(
                    ...
                    reason="Korii Logging channel",
                )

            await self.bot.pool.execute(f"UPDATE log_channels SET {deliver_type}_channel = $1", webhook.url)
            ...
            for e in embeds:
                e.footer.text = (
                    e.footer.text
                    or "" + f"\nCould not deliver to the {deliver_type} channel. Sent here instead!\n"
                    f"Please give me **Manage Webhooks** permissions in #{channel.name}."
                )
                await self.log(e, guild=guild_id, send_to=self.send_to.default)
            ...
            await channel.send(
                ...
                f"\nPlease check if I have the **Manage Webhooks** permissions in all the log channels!"
