...
class JoinLeaveLogs(LoggingBase):
    ...
    async def logging_on_invite_update(self, member: discord.Member, invite: typing.Optional[discord.Invite]):
        ...
        if invite:
            embed.add_field(name="Invited by:",
                            value=f"{discord.utils.escape_markdown(str(invite.inviter))} ({invite.inviter.mention if invite.inviter else 'Unknown'})"
    ...
    async def logging_on_member_remove(self, member: discord.Member):
    ...
    async def logging_on_invite_create(self, invite: discord.Invite):
        assert invite.guild
        ...
        embed = discord.Embed(title="Invite Created", colour=discord.Colour.fuchsia(), timestamp=discord.utils.utcnow(),
                              description=f"**Inviter:** {invite.inviter}{f' ({invite.inviter.id})' if invite.inviter else ''}\n"
                                          ...
                                          f"**Grants Temporary Membership:** {self.bot.tick(invite.temporary)}")
        ...
        await self.log(embed, guild=invite.guild.id, send_to=self.send_to.join_leave)

    @commands.Cog.listener("on_invite_delete")
    async def logging_on_invite_delete(self, invite: discord.Invite):
        assert invite.guild
