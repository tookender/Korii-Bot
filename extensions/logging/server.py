...
class ServerLogs(LoggingBase):
    ...
    async def logger_on_guild_channel_delete(self, channel: guild_channels):
        ...
        embed = discord.Embed(
            ...
            f"\n**Topic:** {discord.utils.remove_markdown(channel.topic or '') if isinstance(channel, TOPIC_CHANNELS) else 'None'}"
                ...
                for perm, value in b_o.items():
                    if value != a_o[perm]:
                        updated_perms.append(
                            f"{str(perm).replace('server', 'guild').replace('_', ' ').title()}: {self.bot.tick(value)} ➜ {self.bot.tick(a_o[perm])}"
                        )
    ...
    async def logger_on_guild_role_create(self, role: discord.Role):
        ...
        embed = discord.Embed(
            ...
            f"**Show Separately:** {self.bot.tick(role.hoist)} • **Color:** {role.color}\n"
    ...
    async def logger_on_guild_role_update(self, before: discord.Role, after: discord.Role):
        ...
        if before.mentionable != after.mentionable:
            ping_update = f"\n**Mentionable:** {self.bot.tick(before.mentionable)} ➜ {self.bot.tick(after.mentionable)}"
            deliver = True
