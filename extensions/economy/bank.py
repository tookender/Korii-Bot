...
class BankCog(EconomyBase):
    ...
    async def deposit(self, ctx, amount: BalanceConverter):
        ...
        await add_bank(self.bot, ctx.author.id, amount)
    ...
    @commands.cooldown(1, 2, commands.BucketType.user)
    async def withdraw(self, ctx, amount: BankConverter):
        if not await has_enough_bank(self.bot, ctx.author.id, amount):
            bank = await get_bank(self.bot, ctx.author.id)

            return await self.send_embed(ctx, text=f"{self.bot.E['no']} | You only have **${bank}**.", color=discord.Color.red())

        await remove_bank(self.bot, ctx.author.id, amount)
