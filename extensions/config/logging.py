...
async def get_wh(channel: discord.TextChannel):
    if channel.permissions_for(channel.guild.me).manage_webhooks:
        ...
        for w in webhooks:
            ...
            return (await channel.create_webhook(name="Korii Logging", avatar=await channel.guild.me.avatar.read())).url
    else:
...
class ChannelsView(discord.ui.View):
    ...
    async def update_message(self, edit: bool = True):
        channels = await self.bot.pool.fetchrow("SELECT * FROM log_channels WHERE guild_id = $1", self.ctx.guild.id)
        embed = discord.Embed(
...
class EventToggle(discord.ui.Button["AllEvents"]):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        q = await self.view.ctx.bot.pool.fetch(
        ...
        self.emoji = self.view.ctx.bot.tick(self.enabled)
        setattr(
...
class LoggingConfig(ConfigBase):
    ...
    async def log_disable(self, ctx: CustomContext):
        ...
        async with ctx.typing():
            try:
                ...
                pass
            channels = await self.bot.pool.fetchrow("DELETE FROM log_channels WHERE guild_id = $1 RETURNING *", ctx.guild.id)
    ...
    async def log_channels(self, ctx: CustomContext):
        ...
        channels = await self.bot.pool.fetchrow("SELECT * FROM log_channels WHERE guild_id = $1", ctx.guild.id)
        embed = discord.Embed(
            title="Logging Channels",
            colour=discord.Colour.blurple(),
    ...
    async def log_auto_setup(self, ctx: CustomContext):
        ...
        async with ctx.typing():
            try:
                ...
                join_leave_webhook = await join_leave_channel.create_webhook(name="Korii logging", avatar=avatar)
                message_channel = await cat.create_text_channel(name="message-log")
                message_webhook = await message_channel.create_webhook(name="Korii logging", avatar=avatar)
                voice_channel = await cat.create_text_channel(name="voice-log")
                voice_webhook = await voice_channel.create_webhook(name="Korii logging", avatar=avatar)
                member_channel = await cat.create_text_channel(name="member-log")
                member_webhook = await member_channel.create_webhook(name="Korii logging", avatar=avatar)
                server_channel = await cat.create_text_channel(name="server-log")
                server_webhook = await server_channel.create_webhook(name="Korii logging", avatar=avatar)
                self.bot.log_channels[ctx.guild.id] = self.bot.log_webhooks(
