...
class DamageView(ui.View):
    ...
    async def full_damage(self, interaction, _):
        embed = create_damage_embed(f"Full Damage Calculator ({self.ticks} ticks)", self.damage_data, multiply=self.ticks)
        await interaction.response.send_message(embed=embed, ephemeral=True)
        
    @ui.button(label="Raw JSON Data", style=ButtonStyle.green)
    async def json_data(self, interaction, _):
        await interaction.response.send_message(f"{'`'*3}json\n{self.damage_data}\n{'`'*3}", ephemeral=True)

def create_damage_embed(title: str, damage_data: dict, multiply: int = 1) -> Embed:
    embed = Embed(title=title)
    embed.set_author(name="Dungeon Quest Helper", url="https://roblox.com/games/2414851778")
